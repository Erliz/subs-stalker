...
const setupFakeServer = ({code = 200, query = emptyQuery, res = defaultResponseText, headers = fakeServerHeader}) => {
...
describe('downloader', () => {
    ...
    describe('download', () => {
        ...
        it('should dispatch error if no content-disposition', (done) => {
            ...
            downloader.download({}, '/tv/anime/');
            eventEmitter.once('subs:download:error', (err) => {
                expect(err).to.be.instanceof(Error).with.property('message').that.string('content-disposition');
                fakeServer.done();
                done();
            })
        });
